...
func describeShape(v any) shapeReport {
	...
	case nil:
		...
		report.SuggestedOptions = map[string]any{}
	...
	case []any:
		...
		report.SuggestedOptions = arraySuggestedOptions(x)
	...
	case map[string]any:
		...
		if t, ok := x["text"].(string); ok && len(x) == 1 {
			report.Kind = "text_blob"
			report.SuggestedPreset = "markdown"
			report.SuggestedOptions = map[string]any{"path": "text"}
			report.Hint = "Source returned text only (wrapped as {text}). Use preset=markdown with options.path=\"text\"."
			_ = t
			break
		}
		report.Kind = "object"
		report.SuggestedPreset, report.SuggestedOptions, report.Hint = objectPresetOptionsAndHint(x)
	default:
		report.Kind = "scalar"
...
func arraySuggestedOptions(arr []any) map[string]any {
	...
	if (hasKey(first, "date") || hasKey(first, "time") || hasKey(first, "timestamp")) && hasNumericField(first) {
...
func objectPresetOptionsAndHint(m map[string]any) (preset string, options map[string]any, hint string) {
	// Find nested arrays.
	...
	if len(arrayKeys) == 1 {
		...
		if len(inner) > 0 {
			if im, ok := inner[0].(map[string]any); ok && (hasKey(im, "date") || hasKey(im, "time") || hasKey(im, "timestamp")) && hasNumericField(im) {
				opts := map[string]any{"seriesPath": k, "x": firstExistingKey(im, []string{"date", "time", "timestamp"}), "y": firstNumericKey(im)}
				return "line_chart", opts, fmt.Sprintf("Source has a time series at %q. Use preset=line_chart with options.seriesPath=%q, options.x=<date|time>, options.y=<numeric>.", k, k)
			}
			if _, ok := inner[0].(map[string]any); ok {
				opts := map[string]any{"path": k, "columns": firstKeys(inner[0].(map[string]any), 6)}
				return "table", opts, fmt.Sprintf("Source has an array of objects at %q. Use preset=table with options.path=%q and options.columns=[...].", k, k)
			}
			return "list", map[string]any{"itemsPath": k}, fmt.Sprintf("Source has an array at %q. Use preset=list with options.itemsPath=%q.", k, k)
		}
		return "list", map[string]any{"itemsPath": k}, fmt.Sprintf("Source has an (empty) array at %q. Use preset=list with options.itemsPath=%q.", k, k)
	}
	// Multiple arrays → let the agent choose; recommend table by default.
	if len(arrayKeys) > 1 {
		return "table", map[string]any{}, fmt.Sprintf("Source has multiple arrays (%s). Pick one and use preset=table with options.path=<key>.", strings.Join(arrayKeys, ", "))
	}
	// No arrays — try to find a numeric field for a metric.
	if numKey := firstNumericKey(m); numKey != "" {
		return "metric", map[string]any{"path": numKey}, fmt.Sprintf("Source is a flat object. Use preset=metric with options.path=%q (or pick another numeric field).", numKey)
	}
	// No arrays, no numerics — probably a label-style record.
	return "markdown", map[string]any{}, "Source is a flat object with no numeric fields. Use preset=markdown, or preset=metric with options.path=<field>."
}
...
func firstExistingKey(m map[string]any, keys []string) string {
	for _, k := range keys {
		if _, ok := m[k]; ok {
			return k
		}
...
func firstKeys(m map[string]any, limit int) []string {
	keys := make([]string, 0, len(m))
	for k := range m {
		keys = append(keys, k)
	}
